...
DATA_DIR=""
if [[ -n "${QAG_OFFLINE_HOST:-}" && \
    -n "${QAG_OFFLINE_INPUT:-}" ]]; then
  case "${QAG_OFFLINE_INPUT}" in
    txt|json) ;;
    *) die "QAG_OFFLINE_INPUT must be txt or json" ;;
  esac
  case "${QAG_OFFLINE_HOST}" in
    [Rr]epo|[Ll]inux)
      ...
      ;;
    [Ww]indows|[Ww][Ss][Ll])
      _dw="${QAG_WINDOWS_DOWNLOADS_ROOT:-/mnt/c/Users/tyewhong/Downloads}"
      export DATA_DIR="${_dw}/${QAG_OFFLINE_INPUT}"
      ;;
    [Dd]ata)
...
def _compute_data_dir(env: dict) -> str:
    ...
    clean = {
        ...
        if not k.startswith("QAG_")
    }
    clean.update(env)
    clean.setdefault("HOST_DIR", "/fake/qag/repo")
    proc = subprocess.run(
...
class OfflinePresetDataDirTest(unittest.TestCase):
        ...
        out = _compute_data_dir(
            {
                ...
                "QAG_SHARED_DATA_ROOT": "/srv/shared/Data",
            }
        )
    ...
    def test_preset_3_repo_txt_default_root(self) -> None:
        out = _compute_data_dir(
            {
                "HOST_DIR": "/home/me/qag",
                "QAG_OFFLINE_HOST": "repo",
                "QAG_OFFLINE_INPUT": "txt",
            }
        )
        self.assertEqual(out, "/home/me/qag/data")
        ...
        out = _compute_data_dir(
            {
                ...
                "QAG_OFFLINE_HOST": "repo",
                "QAG_OFFLINE_INPUT": "txt",
                "QAG_REPO_DATA_ROOT": "/opt/qag-data",
            }
        )
        self.assertEqual(out, "/opt/qag-data")
