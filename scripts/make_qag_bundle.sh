...
# ============================================================================
# make_qag_bundle.sh  --  Build the qag_bundle.tar.gz (code + configs)
# ============================================================================
#
# Produces ONE archive (qag_bundle.tar.gz) containing everything the
...
#   * scripts/lora/                                (host LoRA SFT/DPO finetune)
...
#   * docker-compose.vllm-redserver.yml             (redserver / gpuserver URLs)
#   * docker-compose.vllm-external.yml              (compatibility alias)
#   * docker-compose.vllm-qwen3-14b-gen.yml         (optional Qwen3-14B generator)
#   * config/config.vllm.redserver.yaml             (external vLLM profile)
#   * Dockerfile, Dockerfile.kubeflow              (for rebuilds on-site)
#   * docs/ (includes HANDOVER.md, OFFLINE_SETUP_GUIDE.md), README.md, .env
#   * docs/vscode-extensions/bierner.markdown-mermaid-*.vsix  (offline VS Code)
...
#   * qag-v1.tar                 Docker image for ollama profile
#   * qag-kubeflow.tar           Docker image for kubeflow profile
...
#   bash scripts/make_qag_bundle.sh --include-data    # also bundle data/
#
# Output (default /data/tyewhong/qag/ — override with QAG_ARCHIVE_DIR):
#   qag_bundle.tar.gz          (extracts to qag_host/)
#   qag_bundle.tar.gz.sha256
# ============================================================================
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<USAGE
Usage: bash scripts/make_qag_bundle.sh [--include-data]
...
BUNDLE_NAME="qag_host"
STAGING_DIR="${REPO_DIR}/.bundle_staging_${BUNDLE_NAME}"
OUTPUT_TGZ="${ARCHIVE_DIR}/qag_bundle.tar.gz"
...
main() {
  ...
  if [[ -f "$REPO_DIR/scripts/offline/dotenv.redserver.template" ]]; then
    cp "$REPO_DIR/scripts/offline/dotenv.redserver.template" \
      "$STAGING_DIR/scripts/offline/dotenv.redserver.template"
  fi
  for _offline_sh in fetch_vscode_mermaid_vsix.sh install_vscode_diagram_preview.sh apply_redserver_env.sh extract_qag_bundle.sh; do
    if [[ -f "$REPO_DIR/scripts/offline/$_offline_sh" ]]; then
      cp "$REPO_DIR/scripts/offline/$_offline_sh" \
        "$STAGING_DIR/scripts/offline/$_offline_sh"
      chmod +x "$STAGING_DIR/scripts/offline/$_offline_sh"
    fi
  done
  if [[ -f "$REPO_DIR/docs/vscode-extensions/extensions.json" ]]; then
    mkdir -p "$STAGING_DIR/.vscode"
    cp "$REPO_DIR/docs/vscode-extensions/extensions.json" \
      "$STAGING_DIR/.vscode/extensions.json"
    cp "$REPO_DIR/docs/vscode-extensions/settings.json" \
      "$STAGING_DIR/.vscode/settings.json"
  fi
  ...
  if [[ -f "$REPO_DIR/docker-compose.vllm-redserver.yml" ]]; then
    _strip_build_block < "$REPO_DIR/docker-compose.vllm-redserver.yml" \
      > "$STAGING_DIR/docker-compose.vllm-redserver.yml"
    cp "$STAGING_DIR/docker-compose.vllm-redserver.yml" \
      "$STAGING_DIR/docker-compose.vllm-external.yml"
  elif [[ -f "$REPO_DIR/docker-compose.vllm-external.yml" ]]; then
    _strip_build_block < "$REPO_DIR/docker-compose.vllm-external.yml" \
      > "$STAGING_DIR/docker-compose.vllm-redserver.yml"
    cp "$STAGING_DIR/docker-compose.vllm-redserver.yml" \
      "$STAGING_DIR/docker-compose.vllm-external.yml"
  fi
  if [[ -f "$REPO_DIR/docker-compose.vllm-qwen3-14b-gen.yml" ]]; then
    _strip_build_block < "$REPO_DIR/docker-compose.vllm-qwen3-14b-gen.yml" \
      > "$STAGING_DIR/docker-compose.vllm-qwen3-14b-gen.yml"
  fi
  ...
  info "Copying helper scripts (conversion, utils, lora, docker-entrypoint) ..."
  ...
  if [[ -d "$REPO_DIR/scripts/lora" ]]; then
    cp -a "$REPO_DIR/scripts/lora"            "$STAGING_DIR/scripts/lora"
    find "$STAGING_DIR/scripts/lora" -name '*.sh' -exec chmod +x {} + 2>/dev/null || true
  fi
  ...
  if [[ -f "$REPO_DIR/scripts/download_vllm_generator_qwen3_14b_fp8.sh" ]]; then
    cp "$REPO_DIR/scripts/download_vllm_generator_qwen3_14b_fp8.sh" \
      "$STAGING_DIR/scripts/"
    chmod +x "$STAGING_DIR/scripts/download_vllm_generator_qwen3_14b_fp8.sh"
  fi
  ...
  if [[ -f "$REPO_DIR/config/config.vllm.redserver.yaml" ]]; then
    cp "$REPO_DIR/config/config.vllm.redserver.yaml" "$STAGING_DIR/config/"
  fi
  ...
  _vsix_dir="${REPO_DIR}/docs/vscode-extensions"
  if ! ls "${_vsix_dir}/"*.vsix >/dev/null 2>&1; then
    if [[ -f "${REPO_DIR}/scripts/offline/fetch_vscode_mermaid_vsix.sh" ]] \
        && command -v curl >/dev/null 2>&1; then
      info "Fetching VS Code Mermaid VSIX (redserver has no internet) ..."
      bash "${REPO_DIR}/scripts/offline/fetch_vscode_mermaid_vsix.sh" || true
    fi
  fi
  if ! ls "${_vsix_dir}/"*.vsix >/dev/null 2>&1; then
    die "Missing ${_vsix_dir}/*.vsix — redserver cannot download it." \
      "On this online build host run:" \
  ...
  echo "  Extracts : qag_host/"
  _vsix_in="$(ls "${REPO_DIR}/docs/vscode-extensions/"*.vsix 2>/dev/null \
    | sort -V | tail -n1 || true)"
  if [[ -n "$_vsix_in" ]]; then
    echo "  VSIX     : qag_host/docs/vscode-extensions/$(basename "$_vsix_in")"
  fi
  echo ""
  echo "This bundle does NOT contain the Docker image or model weights."
  echo "  (includes scripts/lora/ for host LoRA finetune)"
