#!/usr/bin/env python3
"""Build a polished deck explaining QAGRedo per-document JSON output fields.
...
DEFAULT_SAMPLE = ROOT / "examples" / "sample_qagredo_doc_analysis.json"
DEFAULT_OUT = ROOT / "docs" / "QAGRedo_Output_Fields_Overview.pptx"
...
LANGGRAPH_DETAIL_IMG = (
    ROOT / "docs" / "qagredo_langgraph_pipeline_detailed.png"
)
FULL_PIPELINE_IMG = ROOT / "docs" / "qagredo_full_pipeline_flow_16x9.png"
...
def _add_hero(prs: Presentation, doc_id: str, grade: str) -> None:
    ...
    kf.clear()
    kp = kf.paragraphs[0]
    kp.text = "QAGRedo · Per-document deliverable"
    kp.font.size = Pt(14)
    ...
    _footer(slide, "QAGRedo output schema overview")
    ...
    for candidate in chain:
        if candidate.is_file():
            if candidate.resolve() == LANGGRAPH_DETAIL_IMG.resolve():
                note = (
                    "Detail flow: "
                    "docs/qagredo_langgraph_pipeline_detailed.png "
                    "(source: qagredo_langgraph_pipeline_detailed.dot)"
                )
            else:
                note = (
                    "Overview: "
                    "docs/qagredo_full_pipeline_flow_16x9.png "
                    "(source: qagredo_full_pipeline_flow.dot)"
                )
    ...
    if resolved is not None:
        ...
        _bullets_in_slide(
            ...
            [
                ...
                "docs/qagredo_langgraph_pipeline_detailed.png, or",
                "docs/qagredo_full_pipeline_flow_16x9.png",
            ],
            0.65,
            1.55,
...
def _add_closing(prs: Presentation) -> None:
    ...
    cmds = [
        ...
        "",
        "Rebuild this deck:",
        "python3 scripts/utils/build_qagredo_output_fields_ppt.py \\",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build QAGRedo output-field overview slides."
    )
    parser.add_argument(
        "--analysis",
        type=Path,
