#!/usr/bin/env python3
"""Build editable PPTX: QAGRedo pipeline (2 slides — core + save)."""
...
def _build_slide_core(prs: Presentation) -> None:
    ...
    tp.text = "QAGRedo pipeline — core loop (per document)"
    tp.font.size = Pt(17)
    tp.font.bold = True
    ...
    _footnote(
        ...
        "python3 scripts/utils/build_qagredo_pipeline_flowchart_pptx.py",
    )
...
def _build_slide_save(prs: Presentation) -> None:
    ...
    title.line.fill.background()
    tp = title.text_frame.paragraphs[0]
    tp.text = "QAGRedo — save & output shaping (after slot loop)"
    tp.font.size = Pt(17)
    tp.font.bold = True
    tp.font.name = "Calibri"
    tp.font.color.rgb = _TEXT
    ...
    _footnote(
        ...
        "build_qagredo_pipeline_flowchart_pptx.py",
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Build QAGRedo pipeline flowchart PPTX (2 slides).",
    )
    root = Path(__file__).resolve().parents[2]
    default_out = (
        root / "docs" / "architecture" / "diagrams" /
        "QAGRedo_Pipeline_Flowchart_editable.pptx"
    )
    ap.add_argument(
        "-o",
        "--output",
