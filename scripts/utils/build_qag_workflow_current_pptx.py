#!/usr/bin/env python3
"""
Build editable PPTX for QAG workflow (current per-document flow).

Native shapes + connectors — editable in PowerPoint.
Mirrors docs/architecture/diagrams/qag_workflow_current.dot.
"""
...
def build(output: Path) -> None:
    ...
    tp.text = (
        "QAG workflow — per-document execution (editable)"
    )
    tp.font.size = Pt(20)
    tp.font.bold = True
    ...
    fp.text = (
        ...
        "python3 scripts/utils/build_qag_workflow_current_pptx.py"
    )
    fp.font.size = Pt(8.5)
...
def main() -> None:
    ...
    default_out = Path(
        ...
        "qag_workflow_current_editable.pptx"
    )
    ap.add_argument(
        "-o",
        "--output",
