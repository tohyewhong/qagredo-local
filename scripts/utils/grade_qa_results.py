...
def main():
    ...
    if not qa_files:
        ...
        print(
            "  python grade_qa_results.py output/openai/gpt-4/2025-11-17/qa_results_2docs.json"  # noqa: E501
        )
        return
    ...
    try:
        ...
        print("(Calling configured hallucination method — typically LLM judge...)")
        print()

        graded_results = grade_qa_results(qa_results, method="llm")
        ...
        # Save graded results
        output_path = (
            file_path.parent / f"{file_path.stem}_graded{file_path.suffix}"
        )
        ...
        confidences = [
            r.get("overall_confidence", 0.0) for r in graded_results
        ]
        ...
        for grade in ["A", "B", "C", "D", "F"]:
            ...
            percentage = (
                (count / len(graded_results)) * 100 if graded_results else 0
            )
