#!/usr/bin/env python3
"""
Multi-format input normalizer for QAG.

Converts: pdf | txt | doc | docx | pptx | xlsx | csv | json | jsonl
Outputs: JSONL where each line is a normalized document record that QAG can ingest.  # noqa: E501

Canonical record schema (per line):
  - id
  - title
  - content  (preferred by QAG)
...
Examples:
  /home/tyewhong/qag/.venv/bin/python scripts/conversion/convert_to_qag_jsonl.py --input data/dev-data.jsonl --output data/normalized.jsonl  # noqa: E501
  /home/tyewhong/qag/.venv/bin/python scripts/conversion/convert_to_qag_jsonl.py --input data/my.pdf --output data/my.jsonl  # noqa: E501
"""
...
def _repo_root() -> Path:
    """
    Find the QAG repo root even when this script lives under /scripts/.
...
def _flatten_press_doc(
    ...
    with ``content`` at the top level so the normal QAG text
...
def _extract_text_field(document: Dict[str, Any]) -> str:
    """
    Extract text content from a document dict, mirroring QAG’s tolerance.
    """
    text_fields = [
        "content",
        "text",
...
def _canonicalize_record(
    ...
    *,
    ...
    rec = {
        ...
        "content": content,
        "text": content,  # explicit alias; QAG prefers `content` but accepts `text`  # noqa: E501
        "source": str(source),
