...
- When `answerability_strict: false` (`ollama` / `kubeflow`): those slots are
  **kept** in `qa_pairs` for `--minimise-bad` (same as other gate failures).
...
for `--minimise-bad` unless `answerability_strict: true` (**`vllm`**) or
...
(**`vllm`**) omits failed pre-check, gate, and insufficient-info slots.
...
flowchart TD
  ...
  subgraph SLOTLOOP["Per-slot loop"]
    ...
    GATE -->|Yes| SLOTDONE[Slot complete]
...
3. SLOT LOOP (run_qa_pipeline.py + utils/answer_generator.py) — see §3.4
   ...
   |   |   reject_ungrounded_after_retries is true
...
question_generation:
  ...
  validation:
    ...
    # answerability_strict: false ollama/kubeflow; true vllm (omit bad slots)
...
|-------|-----------------|---------|
| **Self-correction** | Questions undergo grounding, comprehensiveness, and answerability checks (regenerate up to configured limits). Per-slot answerability pre-check skips wasted answer+judge calls. Answers use per-slot retries (`max_answer_attempts`) plus optional replacement questions per slot (`max_question_regeneration_rounds`), with a coverage rewrite pass and grounding gate | 2.6, 2.7, 2.8, 3.4, 3.5 |
...
- `enable_answerability_check: true` (all profiles) adds question-stage and
  per-slot pre-checks; `answerability_strict: true` on **`vllm`** omits failed
  slots from saved `qa_pairs`.
